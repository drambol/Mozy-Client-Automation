...
class WindowsConfirmReplaceMachineElement(WindowsGUIBaseElement):

    def __init__(self, name, matcher, element_wait_time=None):
        self.title = "Replace computer *"

        self._element_wait_time = element_wait_time or 5

        # self._app = Application().connect(title_re=self.title)

        super(WindowsConfirmReplaceMachineElement, self).__init__(name, matcher, self.element_wait_time)
