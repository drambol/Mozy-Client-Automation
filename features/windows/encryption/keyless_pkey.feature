Feature: Windows client activate with keyless(email&password)
  ...
  @smoke @keyless @KAL-022 @gui @gui-smoke @pkey
  Scenario: Windows keyless activate existing account using private personal key from GUI
    ...
    When I keyless activate windows gui with
      ...
      | clientqa_pro_pkey@mozy.com | Test_1234 |             | pkey         | test1234       | mozypro | QA12 |
...
#    Then Windows state is "Restoring"
  ...
  @e2e-win @smoke @keyless @KAL-023 @gui @gui-smoke @pkey
  Scenario: Windows keyless activate new account using pkey from GUI
    When I create 10 test files with
      | file_dir | file_prefix | file_size | file_ext | file_name |
      | smoke    | smoke       | 10000     | dat      | testdata  |
