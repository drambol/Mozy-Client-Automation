Feature: Windows client activate with keyless(email&password)
  ...
  @e2e-win @smoke @keyless @KAL-047 @gui @gui-smoke @changeencryption
  Scenario: Windows keyless activate new account using random key from GUI, then change encryption type to KMIP.
