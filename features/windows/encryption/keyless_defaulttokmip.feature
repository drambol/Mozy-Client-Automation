Feature: Windows client activate with keyless(email&password)
  ...
  @e2e-win @smoke @keyless @KAL-040 @gui @gui-smoke @changeencryption
  Scenario: Windows keyless activate new account using default key from GUI, then change encryption type to KMIP.
